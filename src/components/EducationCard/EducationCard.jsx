...
const EducationCard = ({ education }) => {
  ...
  return (
    <div
      ...
      className="eduBox bg-white relative rounded-[10px] sm:rounded-[1.25rem] lg:rounded-[1.875rem] xl:rounded-[3.125rem] gap-2 flex flex-col md:flex-row items-center justify-center md:justify-between text-center md:text-left p-2 md:p-10 w-full"
    >
      <div className="eduBoxInfo flex flex-col md:flex-row items-center gap-2">
        <span
          onClick={toggleExpand}
          className="bg-gray rounded-[10px] h-[3.125rem] w-[3.125rem] inline-flex items-center justify-center"
        >
            ...
            <Fragment>
              <span
                dangerouslySetInnerHTML={{
                  __html: education?.short_description_preview,
                }}
              />
            </Fragment>
          )}
          {/* {expand ? <TagsDisplay tags={education?.tags} /> : <></>} */}
        </div>
      </div>
    </div>
  );
