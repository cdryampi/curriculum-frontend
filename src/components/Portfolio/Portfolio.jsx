      ...
      <section className="portWrap py-[4.5rem] md:py-[5.5rem] lg:py-[6.5rem] xl:py-[7.5rem relative w-full">
        <div className="container mx-auto">
          <div className="flex flex-col items-center py-8">
            <p className="text-red-400 mb-3">Error al cargar los proyectos.</p>
      ...
      <div className="container mx-auto">
          ...
          {portfolio && portfolio.map((item) => (
            <div key={`${item?.title}-${item?.id}`} className="w-full md:w-[41.57%] px-2 mb-6">
              <div className="portBox group relative overflow-hidden rounded-lg sm:rounded-xl lg:rounded-2xl xl:rounded-3xl w-full">
                ...
                <img className="w-full max-h-[550px] object-contain transition-all duration-500 ease-in-out" src={item?.image?.file} alt={item?.title} loading="lazy" />
                <div className="projInfo absolute left-0 top-3/4 group-hover:top-1/2 -translate-y-1/2 p-5 w-full text-center opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 ease-in-out z-[2]">
                  <h3 className="text-white text-lg md:text-xl font-bold font-poppins">
                    <Link to={item?.link} title={item?.title} target="_blank" rel="noopener noreferrer">{item?.title}</Link>
                  </h3>
                  <div className="text-white font-nunito text-base md:text-lg mt-1" dangerouslySetInnerHTML={{ __html: item?.description }}></div>
                  <Link className="inline-flex items-center justify-center border-2 sm:border-4 border-white/25 hover:border-white rounded-lg h-10 w-10 sm:h-14 sm:w-14 mt-5 transition-all duration-300 ease-in-out" to={item?.link} title={item?.title} target="_blank" rel="noopener noreferrer">
        ...
        <div className="viewAll text-center w-full mt-10 md:mt-[3.125rem] lg:mt-[4.0625rem]">
          <p className="text-desc2 font-NunitoSans text-[1rem] md:text-[1.125rem]">
            ...
            <Link className="text-accent font-Poppins font-medium hover:text-accent2 underline underline-offset-8" to={GIT_HUB_URL} title="Haz click aquí para ver más" target="_blank" rel="noopener noreferrer">Haz click aquí para ver más.</Link>
          </p>
        </div>
      </div>
    </section>
