...
import useLaboralExperience from "../../hooks/UseLaboralExperienceListHook"
import { TagsDisplay } from "../Tag"
import { WorkExpSkeleton } from "../Skeleton"
    ...
    <section className="workExpWrap py-[4.5rem] md:py-[5.5rem] lg:py-[6.5rem] xl:py-[7.5rem] relative w-full" id="workExperience">
      ...
      <div className="container sm:container md:container lg:container xl:max-w-[86.875rem] mx-auto">
        ...
        {(!workExp || workExp.length === 0) ? (
          ...
          <div className="sliderWrap w-full max-w-full relative lg:mt-0 2xl:-mt-24 section-fade-in">
            <Swiper
              slidesPerView={3}
              spaceBetween={0}
              centeredSlides={true}
              loop={workExp.length > 3}
              direction={"vertical"}
              pagination={{ clickable: true }}
              modules={[Pagination]}
              breakpoints={{
                315: { slidesPerView: 1, direction: "horizontal", centeredSlides: false },
                766: { slidesPerView: 2, direction: "horizontal", centeredSlides: false, spaceBetween: 30 },
                1210: { slidesPerView: 2, direction: "horizontal", centeredSlides: false, spaceBetween: 30 },
                1280: { slidesPerView: 3, direction: "vertical", centeredSlides: true },
              }}
              className="mySwiper"
            >
              {workExp.map((item, i) => (
                <SwiperSlide className="slideItem md:m-3 xl:m-4 lg:m-3 sm:m-2 w-full" key={`${item.title}-${i}`}>
                  <div className="expBox lg:p-0 xl:px-[6.875rem] relative w-full">
                    <div
                      ...
                      style={{ backgroundImage: `url(${item?.logo_empresa_fondo?.image_for_mobile_url})` }}
                    ></div>
                    <div className="grid gap-[1.875rem] xl:grid-cols-2 lg:grid-cols-1 grid-cols-1 items-center">
                      <div className="gridItem xl:order-1">
                        <div className="expImg relative overflow-hidden rounded-[10px] sm:rounded-[1.25rem] lg:rounded-[1.875rem] xl:rounded-[3.125rem] max-h-[14rem] opacity-0">
                          <SafeImage className="w-full" src={item?.logo_empresa_fondo?.image_for_mobile_url} alt={item?.logo_empresa_fondo?.title} loading="lazy" decoding="async" />
                          <SafeImage className="absolute bottom-1/3 left-1/2 transform -translate-x-1/2 min-w-[100px] min-h-[100px] max-w-[200px] max-h-[200px] object-contain" src={item?.logo_empresa?.file} alt={item?.logo_empresa?.title} loading="lazy" decoding="async" />
                        </div>
                      </div>
